...
public class Server {
    ...
    private static ArrayList<Socket> clientSocket; //сокет для общения
    private static ServerSocket server; // серверсокет
    private static ArrayList<BufferedReader> in; // поток чтения из сокета
    private static ArrayList<BufferedWriter> out; // поток записи в сокет
        ...
        Thread playerListener1 = new Thread(() -> {
            try {
                ...
                for(int i = 0; i <2; i++) {
                    ...
                    out.get(i).write(direction.get(0) + " " + direction.get(1) + "\n");
